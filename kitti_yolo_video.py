...
for frame_idx, frame_name in enumerate(frames[:300]):
    ...
    for trk in tracks:
        ...
        radius = int(50 + 120*risk)

        cv2.circle(risk_map,(cx,cy),radius,risk,-1)
        ...
        # future frame generator object storage
    ...
    for i in range(len(futures["straight"])):

        s = futures["straight"][i]
        l = futures["left"][i]
        r = futures["right"][i]

        display = np.hstack((s,l,r))

        cv2.imshow(
            "Future Hypotheses (Straight | Left | Right)",
            display
        )
